...
export const editEdge = (edge: Edge) => {
  // 检测edge是否存在
  const isExist = edgesState.edges.find(
    (item) => item.edgeKey === edge.edgeKey,
  );
  if (isExist) {
    // 执行修改逻辑
    const editEdge = edge;
    const editEdgeIndex = edgesState.edges.findIndex(
      (item) => item.edgeKey === editEdge.edgeKey,
    );
...
export const addVariableBykey = (edgeKey: string, variableName: string) => {
  const edgeIndex = edgesState.edges.findIndex(
    (item) => item.edgeKey === edgeKey,
  );
